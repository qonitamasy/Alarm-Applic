...
class AlarmReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        const val TYPE_SECOND_TIME = 0
        const val TYPE_REPEATING = 1

        const val EXTRA_MESSAGE = "message"
        const val EXTRA_TYPE = "type"

        private const val ID_SECONDTIME = 100
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val title = if (type == TYPE_SECOND_TIME) "One Time Alarm" else "Repeating Alarm"
        val notifId = if (type == TYPE_SECOND_TIME) ID_SECONDTIME else ID_REPEATING
    ...
    private fun showAlarmNotification(
        ...
        val builder = NotificationCompat.Builder(context, channelId)
            .setSmallIcon(R.drawable.ic_second_time_alarm)
    ...
    fun setOneTimeAlarm(context: Context, type: Int, date: String, time: String, message: String) {
        ...
        val pendingIntent = PendingIntent.getBroadcast(context, ID_SECONDTIME, intent, 0)
    ...
    fun cancelAlarm(context: Context, type: Int) {
        ...
        val requiresCode = when(type){
            TYPE_SECOND_TIME -> ID_SECONDTIME
            TYPE_REPEATING -> ID_REPEATING
            else -> Log.i("Cancel Alarm", "cancel Alarm: Unknown type of alarm")
        }

        val pendingIntent = PendingIntent.getBroadcast(context, requiresCode, intent,0)
        pendingIntent.cancel()
        alarmManager.cancel(pendingIntent)
        if (type== TYPE_SECOND_TIME){
